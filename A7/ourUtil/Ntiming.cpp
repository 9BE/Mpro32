...
void Ntiming::localDateTime(DateTime* p, long utc, char offsetHour) {
	long days, j;
	int i = START_YEAR;
	int k;
	char hb;
...
double Ntiming::J(int year, char month, char day) {
...
double Ntiming::sun(int year, char month, char day, double lat, double lng,
...
void Ntiming::regSun(Waktu* m, double kini) {
	double temp;
	m->jam = (int) kini;
	temp = kini - (int) kini;
	temp *= 60;
...
void Ntiming::susunMasa(double lat, double lng) {
	double waktu2;
...
String Ntiming::twoDigits(int digits) {
	if (digits < 10) {
...
void Ntiming::ticktock() {
	if (millis() - tunggu >= 1000UL) {
		tunggu = tunggu + 1000;
		utc++;
		susunMasa(_lat, _lng);
	}
...
void Ntiming::setTimeNow(uint8_t jam, uint8_t minit, uint8_t saat,
		uint32_t hari, uint32_t bulan, uint32_t tahun,  double lat, double lng) {

	now.time.jam = jam;
	now.time.minit = minit;
	now.time.saat = saat;
	now.hari = hari;
	now.bulan = bulan;
	now.tahun = tahun + 2000;
	toSecond();
	_lat = lat;
	_lng = lng;
	susunMasa(lat, lng);
...
//	e_dusk
	...
	else if (ZoneTime == e_dawn) {
		ret = "Dawn";
	}
	else if (ZoneTime == e_day) {
		ret = "Day";
	}
	else if (ZoneTime == e_dusk) {
		ret = "Dusk";
	}
